...
#
# Patched cog_download.py to allow third-party repositories like limbo (https://github.com/Marc3842h/limbo)
# Original: https://github.com/appu1232/Discord-Selfbot/blob/master/cogs/cog_download.py
# All credits go to the original author!
# Installation is as easy as to drag & drop this file into your "cogs" directory and to restart the bot.
#
...
class CogDownloading:
    ...
    async def cog(self, ctx):
        """Manage custom cogs from ASCII or limbo. [p]help cog for more information.

        The Appu Selfbot Cog Importable Index (aka ASCII) is a server that hosts custom cogs for the bot.
        Limbo is a third-party repository for Cogs that were not accepted into ASCII.
        [p]cog install <cog> - Install a custom cog from ASCII or Limbo.
        [p]cog uninstall <cog> - Uninstall one of your ASCII or Limbo cogs.
        [p]cog list - List all cogs on ASCII or Limbo.
        [p]cog view <cog> - View information about a cog on ASCII or Limbo.
        [p]cog update - Update all of your ASCII and Limbo cogs.
        If you would like to add a custom cog to ASCII, see http://appucogs.tk
        If you would like to add a custom cog to Limbo, see https://github.com/Marc3842h/limbo
        """
        if ctx.invoked_subcommand is None:
            await ctx.message.delete()
            await ctx.send(
                self.bot.bot_prefix + "Invalid usage. Valid subcommands: `list`, `install`, `uninstall`, `view`, `update`\nDo `help cog` for more information.")

    @cog.command(pass_context=True)
    async def install(self, ctx, cog):
        """Install a custom cog from ASCII or Limbo."""

        def check(msg):
            if msg:
                return (
                                   msg.content.lower().strip() == 'y' or msg.content.lower().strip() == 'n') and msg.author == self.bot.user
            else:
                return False
        ...
        if response.status_code == 404:
            response = requests.get("https://raw.githubusercontent.com/Marc3842h/limbo/master/limbo/cogs/{}.json".format(cog))

            if response.status_code == 404:
                await ctx.send(
                    self.bot.bot_prefix + "That cog couldn't be found on the network. Check your spelling and try again.")
                return

            await ctx.send(self.bot.bot_prefix + "COG has been found on Limbo repositories.")
        else:
            await ctx.send(self.bot.bot_prefix + "COG has been found on ASCII repositories.")
        ...
        if reply.content.lower() == "y":
            ...
            with open("settings/github.json", "r+") as fp:
                ...
                if opt['username'] != "":
                    try:
                        await ctx.send(
                            self.bot.bot_prefix + "Uploading to GitHub. Heroku users, wait for the bot to restart.")
                        await self.github_upload(opt['username'], opt['password'], opt['reponame'], coglink,
                                                     filename)
                    except:
            ...
            try:
                ...
                await ctx.send(
                        self.bot.bot_prefix + "There was an error loading your cog: `{}: {}` You may want to report this error to the author of the cog.".format(
    ...
    async def uninstall(self, ctx, cog):
        ...
        if os.path.isfile("custom_cogs/" + filename + ".py"):
            ...
            if reply.content.lower() == "y":
                ...
                await ctx.send(
                    self.bot.bot_prefix + "Successfully deleted the `{}` cog.".format(found_cog["title"]))
            else:
                await ctx.send(
                    self.bot.bot_prefix + "Didn't delete `{}`: user cancelled.".format(found_cog["title"]))
        else:
            await ctx.send(self.bot.bot_prefix + "You don't have `{}` installed!".format(found_cog["title"]))

    def list_limbo(self, ctx, embed):
        """Lists all cogs on Limbo."""
        site = requests.get('https://github.com/Marc3842h/limbo/tree/master/limbo/cogs').text
        ...
        if installed:
            embed.add_field(name="Installed (Limbo)", value="\n".join(installed), inline=True)
        else:
            embed.add_field(name="Installed (Limbo)", value="None!", inline=True)
        if uninstalled:
            embed.add_field(name="Not installed (Limbo)", value="\n".join(uninstalled), inline=True)
        else:
            embed.add_field(name="Not installed (Limbo)", value="None!", inline=True)
    ...
    async def list(self, ctx):
        ...
        if installed:
            embed.add_field(name="Installed (ASCII)", value="\n".join(installed), inline=True)
        else:
            embed.add_field(name="Installed (ASCII)", value="None!", inline=True)
        if uninstalled:
            embed.add_field(name="Not installed (ASCII)", value="\n".join(uninstalled), inline=True)
        else:
            embed.add_field(name="Not installed (ASCII)", value="None!", inline=True)

        self.list_limbo(ctx, embed)

        await ctx.send("", embed=embed)

    @cog.command(pass_context=True)
    async def view(self, ctx, cog):
        """View information about a cog on ASCII or Limbo."""
        await ctx.message.delete()
        response = requests.get("http://appucogs.tk/cogs/{}.json".format(cog))
        if response.status_code == 404:
            response = requests.get("https://raw.githubusercontent.com/Marc3842h/limbo/master/limbo/cogs/{}.json".format(cog))
            ...
            if response.status_code == 404:
                await ctx.send(self.bot.bot_prefix + "That cog couldn't be found on the network. "
    ...
    # TODO: Implement updating for Limbo repositories
    @cog.command(pass_context=True)
        ...
        for entry in list[2:]:
            ...
            if os.path.isfile("custom_cogs/" + entry + ".py"):
                ...
                try:
                    ...
                    os.remove("custom_cogs/" + filename)
                    await ctx.send(
                        self.bot.bot_prefix + "There was an error updating the `{}` cog: `{}: {}` You may want to report this error to the author of the cog.".format(
                            cog["title"], type(e).__name__, str(e)))
        if failures == 0:
            await msg.edit(content=self.bot.bot_prefix + "Updated all cogs successfully.")
        else:
            await ctx.send(
                self.bot.bot_prefix + "Updated {}/{} cogs successfully.".format(successful, successful + failures))
